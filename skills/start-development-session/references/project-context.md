...
- Design system showcase: static Angular app from `apps/design-system-showcase/` for browser presentation of repository-owned Idealab foundations, components, patterns, and example screens; deployed as Cloud Run service `design-system-showcase` and routed from `desing.idealab.expert` through the existing global HTTPS load balancer.
...
- `design-system-showcase.yml`: builds `apps/design-system-showcase` through Nx, packages the prebuilt static artifact with Docker, pushes `design-system-showcase`, and deploys Cloud Run. `desing.idealab.expert` is routed by the existing global HTTPS load balancer and Cloud DNS zone.
