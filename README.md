...
- **agent** (when `OPENAI_API_KEY` is set) — the LangChain Deep Agent runs the
  ...
  **facilitator**: a fast intake gate (structured-output LLM call, no tools)
  ...
  (max 3 per conversation) instead of solving. Once complete, the Deep Agent
  runs the TRIZ workflow; if its report is long, the chat gets a compressed
  ~80-word summary and the full report lands in the solution card.
